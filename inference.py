...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        # handle the jail case by checking if noisyDistance is None
        if noisyDistance == None:
            allPossible[self.getJailPosition()] = 1.0

        for position in self.legalPositions:
            if (emissionModel[util.manhattanDistance(position,pacmanPosition)] > 0):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        # legal positions to use in the iteration
        legalPos = self.legalPositions

        for position in legalPos:
            # use the implemented functions as directed in the comments above
            ...
            for newPos, probability in newPosDist.items():
                updatedBeliefs[newPos] += (self.beliefs[position] * probability)
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        legalPos = self.legalPositions
        numOfLegalPos = len(legalPos)
        i, list = 0, []
        # while i is in the range of the legal positions append it to the list initialized and set self.particles to the list
        while i < (numOfLegalPos):
            i += 1
            list += legalPos
    ...
    def observe(self, observation, gameState):
        ...
        # handling the 1st the case where the ghost has been captured by pacman
        if (noisyDistance == None):
            j = 0
            while j < (numOfParticles):
                self.particles[j] = self.getJailPosition()
                j += 1
        else:
            for position in legalPos:
                # The true distance
                closestDist = util.manhattanDistance(pacmanPosition, position)
                belief[position] = emissionModel[closestDist] * beliefDist[position]

            belief.normalize()
            # handle the 2nd case as directed in the comments
            #if the particles weight 0 call initializeUniformly function implemented above
            if (belief.totalCount() != 0):
                i = 0
                resampledParticles = []
                while i < (numOfParticles):
                    i += 1
                    resampledParticles.append(util.sample(belief))
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        # A list as instructed to store all particles
        #allParticles , j = [] , 0
        permLegalGhost = list(itertools.product(legalPos, repeat = numOfGhosts))
        """random.shuffle(permLegalGhost)

        for i in range(numOfParticles):
            for p in permLegalGhost:
                if j < numOfParticles:
                    allParticles += p
                    j += 1
                else:
                    break"""
        self.particles = permLegalGhost
    ...
    def observeState(self, gameState):
        ...
        for ghostIndex in range(0, self.numGhosts):
            # if the ghost has not being caught continue with the iterations
            if (noisyDistances[ghostIndex] != None):
                continue
            # initialize a list to record the caught ghost indices
            caughtGhosts = []
            for particle in self.particles:
                #append the list with particles with respect to the ghosts caught using the helper function directed in the comments
                caughtGhosts.append(self.getParticleWithGhostInJail( particle, ghostIndex ))
            self.particles = caughtGhosts

        for particle in self.particles:
            # initialize the initial probability as 1.0
            probability = 1.0
            for i in range(self.numGhosts):
                if (noisyDistances[i] != None):
                    model = emissionModels[i]
                    probability *= model[util.manhattanDistance(particle[i], pacmanPosition)]
            particleCounter[particle] += probability
        # take care of the 2nd condition as directed in the comments above
        # if the recieved weight is 0 use initializeParticles which we implemented above otherwise resample and set self.particles equal to the resampled product
        if (particleCounter.totalCount() != 0):
            i = 0
            resampledParticles = []
            # append the list resampledParticles with the sample of particleCounter until the index is less than the the num of particles
            while i < (self.numParticles):
                i += 1
                resampledParticles.append(util.sample(particleCounter))
            self.particles = resampledParticles
        else:
            self.initializeParticles()
